...
export default function ChatPage() {
  ...
  // --- 1. RAVENS SILENCE PROTOCOL ---
  // This effect listens for the switch. If Ravens is ON, kill the music.
  useEffect(() => {
    if (useOffline && audioRef.current) {
  ...
  useEffect(() => {
    ...
    if (audio) {
      audio.volume = 0.5; 
      // Only auto-play if Ravens Protocol is NOT active
      if (!useOffline) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      {/* 3. VIBE PLAYER - HIDDEN WHEN RAVENS PROTOCOL IS ACTIVE */}
      {!useOffline && (
        <div style={{
            position: 'fixed',
            bottom: '24px',
